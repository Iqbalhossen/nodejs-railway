...
const FixedDeposit = async () => {
    ...
    try {
        ...
        const data = await UserFixedDeposit.find({ expired_time: findDate, status:0 });
        ...
        for (const singleData of data) {
            ...
            const FixedDepositQuery = { _id: new ObjectId(_id) };
            ...
            await UserFixedDeposit.findByIdAndUpdate(FixedDepositQuery, { status: 1 }, option);
            ...
            if (ExitsData === null) {
                ...
                const StoreData = {  user_id: userFind._id, amount: profit_amount,  post_balance: profit_amount, trx_type: '+', trx: RandomTransaction(15), details: `Fixed Deposit`, }
                await TransactionsModels.create(StoreData);
    
            } else {
    
                const StoreData = {  user_id: userFind._id, amount: profit_amount,  post_balance: (parseFloat(ExitsData.post_balance) + parseFloat(profit_amount)), trx_type: '+', trx: RandomTransaction(15), details: `Fixed Deposit`, }
                await TransactionsModels.create(StoreData);
